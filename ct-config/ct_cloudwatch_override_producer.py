...
def override_config_recorder(excluded_accounts, sqs_url, account, event):
    ...
    try:
        ...
        if account == '':
            page_iterator = paginator.paginate(StackSetName ='AWSControlTowerBP-BASELINE-CLOUDWATCH')
        else:
            page_iterator = paginator.paginate(StackSetName ='AWSControlTowerBP-BASELINE-CLOUDWATCH', StackInstanceAccount=account)
